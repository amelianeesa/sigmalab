...
<body class="bg-secondary d-flex flex-column min-vh-100">
    ...
    <div class="container-fluid flex-grow-1 d-flex flex-column p-3">
        @if($personil->file_cv)
            <iframe src="{{ asset('storage/uploads/cv/'.$personil->file_cv) }}" class="w-100 flex-grow-1 rounded shadow" style="min-height: 85vh;" frameborder="0"></iframe>
        @else
            <div class="alert alert-warning text-center mt-5">File CV tidak ditemukan atau belum diunggah.</div>
        @endif
    </div>
</body>
</html>