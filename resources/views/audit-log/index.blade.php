                ...
                <form action="{{ route('audit-log.index') }}" method="GET" class="row g-3 align-items-end">
                    <div class="col-md-3">
                        ...
                        <select name="event" class="form-select form-select-sm">
                            ...
                            <option value="created" {{ request('event') == 'created' ? 'selected' : '' }}>Created (Baru)
                            </option>
                            <option value="updated" {{ request('event') == 'updated' ? 'selected' : '' }}>Updated (Ubah)
                            </option>
                            <option value="deleted" {{ request('event') == 'deleted' ? 'selected' : '' }}>Deleted (Hapus)
                            </option>
                        </select>
                    </div>
                    <div class="col-md-4">
                        ...
                        <input type="text" name="subject_type" class="form-control form-control-sm"
                            placeholder="Contoh: Barang, Kegiatan" value="{{ request('subject_type') }}">
                    </div>
                    <div class="col-md-2">
                        ...
                        <tbody>
                            @forelse($logs as $log)
                                <tr>
                                    ...
                                    <td>
                                        <span
                                            class="fw-bold">{{ $log->causer ? ($log->causer->personil->nama_personil ?? $log->causer->username) : 'Sistem (Otomatis)' }}</span>
                            ...
                            @empty
                                <tr>
                                    <td colspan="6" class="text-center py-4 text-muted">Belum ada rekam jejak yang tercatat.
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
